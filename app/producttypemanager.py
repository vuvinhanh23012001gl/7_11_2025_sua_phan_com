...
class ProductTypeManager:
    ...
    def load_from_file(self):
        ...
        debug_print("📥 Đang tải dữ liệu từ file JSON...")
        ...
        if self.data:
            ...
            for key in self.data.keys():
                ...
                if type_id == -1 or type_name == -1 or xyz == -1:
                    debug_print("❌Không Tìm Thấy 1 Số Dữ liệu Khi Load Trả về False")
                    return
            ...
            self.product_types = {}
            debug_print("❌Data rỗng chưa có dữ liệu")
    ...
    def save_json_data(self, data_file_path:str):
        ...
        try:
            ...
            debug_print(f"✅ Đã lưu dữ liệu JSON vào: {data_file_path}")
        except Exception as e:
            debug_print(f"❌ Lỗi khi lưu dữ liệu JSON: {e}")
    def add_product_type(self,id:str,name:str,xyz:list,description:str="")->bool:
        ...
        if status == 1: # Du lieu da co
            debug_print("Dữ liệu đã có bị trùng ID Không lưu")
            return False
        elif status == 0:
            if(product.check_xyz()):
                debug_print("🔔Kiểm tra trước khi thêm OKE")
                product.Init_path()
                ...
                try:
                    ...
                    debug_print(f"❌ Lỗi khi lưu JSON sau khi thêm: {e}")
                    return False
            else:
                debug_print("❌Lỗi Data Không Hợp Lệ")
                False
        else:
            debug_print("File Trống Cứ Thế Lưu")
            product.Init_path()
            self.product_types[product.type_id] = product
            self.update_data_json()
            debug_print("self.product_types",self.product_types)
    ...
    def get_list_path_master_product_img_name(self,idtype:str)->List[Any]:
        ...
        if idtype is not None and  self.product_types is not None:
            ...
            debug_print("Tên ID hoặc dữ liệu chưa có")
    ...
    def show_all(self):
        ...
        if not self.product_types:
            debug_print("❌ Chưa có loại sản phẩm nào.")
            return
        debug_print("📦 Danh sách loại sản phẩm:")
        for pt in self.product_types.values():
    ...
    def remove_product_in_file_data(self, id: str):
        ...
        if id not in self.product_types:
            debug_print(f"❌ Không tìm thấy ID {id}")
            return False
        ...
        self.load_from_file()
        debug_print("self.product_types",self.product_types)
        debug_print("✅ Đã xóa sản phẩm ID:", id)
        debug_print("Danh sách còn lại:", list(self.product_types.keys()))
        return True
    def add_list_point_to_product(self,type_id:str,x:int,y:int,z:int,brightness:int)->bool:
        if type_id is None or x is None or y is None or z is None or brightness is None:
            debug_print("Giá trị đầu vào khác Null")
            return False
        ...
        if isObject != -1:
            ...
            for arr_list_location in arr_list_locations:
                ...
                if x_location == x and y_location == y and z_location == z:
                     debug_print("Điểm đã tồn tại, không thể thêm điểm trùng.")
                     return False
                # Kiem tra lon hon 0
                if x < 0 or y < 0 or z < 0:
                    debug_print("Giá trị x, y, z phải lớn hơn hoặc bằng 0.")
                    return False
                # Kiem tra co nho hon xyz quy định
                xyz = isObject.get_xyz()
                if x > xyz[0] or y > xyz[1] or z > xyz[2]:
                    debug_print(f"Giá trị x, y, z phải nhỏ hơn hoặc bằng {xyz}.")
                    return False
            ...
            return True
        debug_print("Không tìm thấy ID trong danh sách")
        return False
    ...
    def update_data_json(self)->None:
        ...
        self.data = self.get_file_data()
        self.load_from_file()
        debug_print("Cập nhật lại dữ liệu")
    ...
    def create_file_and_path_img_master(self,idtype:str,name_img:str)->bool:
        ...
        if result != -1:
            ...
            if not path_master:
                debug_print("Không tìm thấy đường dẫn Path")
                return False
        ...
        if isObject!= -1:
            ...
            if path_master is not None :
                ...
                if path_product:
                     ...
                     if(path_img):
                        debug_print("Xóa File ảnh thành công",path_img)
                        status_img_product = ProductTypeManager.object_folder.delete_file(path_img)
                        if not status_img_product:
                            debug_print("Xóa File ảnh sản phẩm không thành công")
                        else:
                            debug_print("Xóa File ảnh sản phẩm thành công")
                     else :
                        debug_print("Không tìm thấy File ảnh lưu sản phẩm xóa ảnh chưa đc")
                else:
                    debug_print("Đường dẫn tới Product_Photo không tồn tại")
                ...
                if status !=False  and status_erase_master != False:
                    ...
                    debug_print("Xóa thành công 4 File")
                    ProductTypeManager.object_folder.create_choose_master(NAME_FILE_CHOOSE_MASTER) # tạo file choose_master nếu tạo rồi thì thôi
                    choose_master_index = ProductTypeManager.object_folder.read_data_from_file(NAME_FILE_CHOOSE_MASTER)# đọc lại file choose master cũ xem lần trước  người dùng chọn gì
                    ...
                    debug_print("Xóa bị False")
                    return False
            else:
                debug_print("File không tồn tại")
                return False
        else:
            debug_print("Không tìm thấy ID")
            return False
    def remove_img_master_index_of_product(self,ID,index):
        ...
        debug_print("Tiến hành xóa ảnh master thứ index")
        list_id = self.get_list_id_product()
        if ID in list_id:
                if index < 0:
                    debug_print("Index phải lớn 0")
                    return False  
                try:      
                   isObject = self.find_by_id(ID)
                   path_master = isObject.get_path_img_master()
                   debug_print("Tìm ảnh có tên trong đường dẫn",path_master)
                   list_file_in_dir_befor_index = os.listdir(path_master)
                   if not list_file_in_dir_befor_index:
                       debug_print("Danh sách rỗng")
                       return False
                   if len(list_file_in_dir_befor_index) <= index:
                       debug_print("Trong thư mục ảnh chứa không index không hợp lệ") # trường hợp này xảy ra khi Data vị trí có nhưng chưa chụp ảnh lưu 
                       return False
                   else:
                       debug_print(f"Tìm ảnh có index :{index}")
                       ...
                       if not os.path.exists(patd_index):
                           ...
                           debug_print("File tồn tại Tiến hành xóa File")
                           os.remove(patd_index)
                           debug_print("Tiến hành đổi tên File ảnh")
                           list_file_in_dir_after_index = os.listdir(path_master)
                           debug_print("Danh sách trước khi xóa",list_file_in_dir_befor_index)
                           debug_print("Danh sách sau khi xóa",list_file_in_dir_after_index)
                           ...
                           debug_print("Đổi tên thành cống")
                           debug_print(f"Xóa file {name_file_index} thành công")
                           return True
                except:
                    debug_print("Xóa lỗi, ko xóa được")
                    return False
        else:
            debug_print("Không tìm thấy ID trong list ID")
            return False
        ...
        if ID in list_id:
            ...
            if arr_point:
                if len(arr_point) <= index:
                    debug_print("Index phải nhỏ hơn số lượng điểm trong sách điểm")
                    return False
                if index < 0:
                    debug_print("Index phải lớn 0")
                    return False  
                try:      
                   ...
                   self.update_data_json()
                   debug_print("Xóa index thành công")
                   return True
                except:
                    debug_print("Xóa lỗi, ko xóa được")
                    return False
            else:
                debug_print("Sản phẩm chưa có list ID")
                return False
    ...
    def remove_all_master_index(self,ID:str,index:int,obj_shape_master):
        ...
        if statuse_erase_img == True and statuse_erase_data == True:
            debug_print("Xóa thành công ảnh master thứ ",index)
            debug_print("Xóa thành công dữ liệu master thú",index)
            if not statuse_erase_master_index:
                debug_print("Xóa không thành công dữ liệu master do....")
            else:
                debug_print("Xóa thành công dữ liệu master")
        debug_print("---------------------------------------------o0o--------------------------------------------------")
